...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nome) || empty($email) || empty($senha) || empty($tipo)) {
        ...
        $sql = "INSERT INTO usuarios (nome, email, setor, senha, permissao) 
                VALUES (:nome, :email, :setor, :senha, :permissao)";
        $stmt = $pdo->prepare($sql);

        $stmt->bindValue(':nome', $nome, PDO::PARAM_STR);
        $stmt->bindValue(':email', $email, PDO::PARAM_STR);
        $stmt->bindValue(':setor', $tipo, PDO::PARAM_STR);
        $stmt->bindValue(':senha', $senhaHash, PDO::PARAM_STR);
        $stmt->bindValue(':permissao', $permissaoStr, PDO::PARAM_STR);
    ...
    <form action="" method="post" class="max-w-md mx-auto flex flex-col gap-4">
        ...
        <div>
            ...
            <div class="flex flex-col gap-2 border border-gray-300 p-3 rounded-md">
                <label>
                    <input type="checkbox" name="permissoes[]" value="gerenciar_alunos" 
                        <?= in_array('gerenciar_alunos', $permissoes) ? 'checked' : '' ?>>
                    Gerenciar Alunos
                </label>
                <label>
                    <input type="checkbox" name="permissoes[]" value="ver_relatorios" 
                        <?= in_array('ver_relatorios', $permissoes) ? 'checked' : '' ?>>
                    Ver Relatórios
                </label>
                <label>
                    <input type="checkbox" name="permissoes[]" value="editar_turmas" 
                        <?= in_array('editar_turmas', $permissoes) ? 'checked' : '' ?>>
                    Editar Turmas
                </label>
                <label>
                    <input type="checkbox" name="permissoes[]" value="administrar_usuarios" 
                        <?= in_array('administrar_usuarios', $permissoes) ? 'checked' : '' ?>>
